...
def sgp4(satrec, tsince, whichconst=None):
     if satrec.method == 'd':
         if isscalar(tsince):
             ...
             r_tup = (rx_arr,ry_arr,rz_arr)
             v_tup = (vx_arr,vy_arr,vz_arr)
             return (r_tup,v_tup)
     ...
     if isscalar(em):
        if (em >= 1.0) or (em < -0.001):  # || (am < 0.95)
            ...
            satrec.error_message = ('mean eccentricity {0:} not within'
                                    ' range 0.0 <= e < 1.0'.format(em))
            satrec.error = 1;
            #  sgp4fix to return if there is an error in eccentricity
            return false, false;
        ...
        if mask_em.any():
            print('Issue with em, ignore for now')
            # satrec.error_message = ('mean eccentricity {0:} not within'
            #                         ' range 0.0 <= e < 1.0'.format(em))
            # satrec.error = 1;
            #  sgp4fix to return if there is an error in eccentricity
     ...
     if isscalar(mrt):
        ...
        mask_mrt = (mrt < 1.0)
        ignore_mask = mask_mrt | mask_em
        if ignore_mask.any():
            print('Issue with mrt, ignore for now')
            # satrec.error_message = ('mrt {0:} is less than 1.0 indicating'
            #                         ' the satellite has decayed'.format(mrt))
            # satrec.error = 6;
        r_arr = array(r)
        r_arr[:,ignore_mask] = _nan
        v_arr = array(v)
        v_arr[:,ignore_mask] = _nan
        r = r_arr
        v = v_arr
